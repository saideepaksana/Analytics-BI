      ...
      await processGridFsFile({
        ...
        onQuarantine: async (quarantineBatch) => {
          const structuralDlqDocs = quarantineBatch.map((row) => ({
            ...
            errorDetails: row.errorDetails || [{ field: null, errorType: 'structural_parse_error', message: 'Structural parse error', value: null }],
          ...
          if (allTransformed) {
            ...
            if (invalidRows.length > 0) {
              const dlqDocs = invalidRows.map((row, index) => ({
                ...
                errorDetails: row.errorDetails || [{ field: null, errorType: 'validation_failed', message: 'Validation failed', value: null }],
